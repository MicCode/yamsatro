...
func click():
	SoundPlayer.play_sfx("rolls/roll-stop-6.ogg", {pitch = 1.0, pitch_variation = 0.05})
...
func tada():
	SoundPlayer.play_sfx("tada.ogg", {volume = -6.0})


func _one_of(base_name: String, n: int) -> String:
	return base_name + "-" + str(randi_range(1, n)) + ".ogg"
