...
func _ready() -> void:
    ...
    PanelManager.register(PanelManager.PanelNames.HomeMenu, %HomeMenu)
    ...
    PanelManager.set_main(PanelManager.PanelNames.HomeMenu)
...
func show_menu():
    %ScoresMenuOverlay.update()
    PanelManager.show(PanelManager.PanelNames.ScoresMenuOverlay).connect("finished", func():
        %Background.hide()
    )

func hide_menu():
    %Background.show()
    PanelManager.hide(PanelManager.PanelNames.ScoresMenuOverlay).connect("finished", func():
        %Background.show()
    )
...
func _on_home_menu_start_button_pressed() -> void:
    PanelManager.show(PanelManager.PanelNames.GameUi).connect("finished", func():
        PanelManager.set_main(PanelManager.PanelNames.GameUi)
        %GameUI.start_game()
    )
